...
| 配置分发机制：loaddata 仅首次部署灌(v20) | 【推翻 2026-08-17 事故 BUG-2026-0817-01（启动链 loaddata 条件判断容器内误判→撞唯一约束→容器崩溃）】服务器 27281 vs 本机 955 根因=配置存数据库不随 git 走 + 启动链每次 loaddata 用旧基线覆盖；治本：①docker-compose 启动链 loaddata 条件化（modeling.Domain 存在则跳过，仅首次部署空库灌配置快照）——配置以数据库为准，界面改配置立即生效、重启不还原；②data_dump.json 重新导出为正确基线（从本机 dev.db，排除 auth.user/authtoken/userprofile/archive 等敏感+业务数据，PYTHONUTF8=1 防 GBK）+解除 .gitignore 入库（决策变更，用户确认）——git pull 自动同步基线，新服务器首次部署出厂即正确；③服务器 DB 补 3 处（cfg2/cfg6 conditions+join_type、FM9 inner）对齐本机 | 2026-08-17 |
| 配置分发机制：启动链零数据操作，基线仅首次部署手动灌(v21) | 【推翻 2026-08-17 v20 loaddata 条件化】v20 上线即事故：服务器 pull+重建后 backend 容器 Restarting 崩溃（loaddata 条件命令在容器内误判走 else → 灌基线撞唯一约束 (1,1,2,33) 已存在 → IntegrityError → 启动链中断 → 前端全挂假象，数据库数据未丢）；教训=启动链双分支逻辑必须容器内实测后才可上线、loaddata 非事务撞约束部分导入需核查。治本：①compose 启动链彻底移除 loaddata（migrate→init_admin→collectstatic→gunicorn），启动链零数据操作；②data_dump.json 基线仅用于首次部署**手动**导入（compose 注释写明命令 `docker compose exec backend python manage.py loaddata /app/data_dump.json`）；③基线入库+解除 gitignore 维持（v20 ②项继续有效，git pull 同步基线） | 2026-08-17 |
...
## 架构级决策：配置分发机制 v20 推翻——启动链零数据操作，基线仅首次部署手动灌（2026-08-17 事故修复）

- **背景**：v20（loaddata 条件化）上线即事故——服务器 git pull + up --build 后 backend 容器 `Restarting (1)` 循环崩溃，前端全挂（用户误以为「域管理的东西都没有了」）。日志铁证：loaddata 走了 else 分支 → `IntegrityError: Could not load modeling.FieldMapping(pk=11): duplicate key (1,1,2,33) already exists` → 启动链 && 中断 → gunicorn 未起
- **根因（两层）**：①条件命令 `python manage.py shell -c '...sys.exit(0 if Domain.objects.exists() else 1)'` 在容器内实际走了 else（exists() 返回 False 或退出码非 0，**未在容器内复现确认**——本地模拟验证只测了 migrate+loaddata 导入路径，从未实测 if 分支=验证缺口）；②服务器库已有同键 FM（pk≠11）→ loaddata 插入 pk=11 撞唯一约束
- **决策（用户拍板方案A，commit 99c6c86）**：①启动链彻底移除 loaddata，恢复纯启动链 migrate→init_admin→collectstatic→gunicorn——**启动链零数据操作**，杜绝任何自动灌/自动覆盖风险；②data_dump.json 基线仅用于首次部署**手动**导入（compose 注释写明命令）；③基线入库+解除 gitignore 维持 v20 ②项（git pull 同步基线，新服务器首次部署出厂正确）
- **实证支撑**：YAML 解析 OK（command 展开为纯启动链）；数据库未丢（报错证明配置都在）；loaddata 撞约束前部分对象已导入（同 pk 被 UPDATE 成本机配置=正确方向），恢复后需核查残留
- **教训（rule 级）**：启动链双分支逻辑必须在目标容器环境实测后才可上线；loaddata 非事务、撞约束会部分导入；「页面全空」先查容器状态+日志，禁止直接走恢复数据流程
- **回执**：闸[✓] 记[✓] 拓[✓] 测[✓]（无新增路径——YAML 解析验证 + 事故取证链）

## 架构级决策：配置分发机制 loaddata 仅首次部署灌（2026-08-17 方向锁定，已实施；v20，已被 v21 推翻）

- **背景**：服务器 diag_precombine 输出证实 27281 vs 955 根因链——①用户本机界面配的 64 条限制+inner join（cfg2 08-17/cfg6 08-13）只写进 dev.db，配置存数据库不随 git 走，服务器 PostgreSQL 从未收到（updated_at=08-11）；②服务器 data_dump.json 是 08-11 部署基线且结构性过时（FieldMapping/DetailTableConfig 记录无 join_type 字段——该字段后续新增，loaddata 导入取默认 left）；③唯一 inner 挂载 FM11 无条件 → kept=209,123 全量 → 主表 239,504 行匹配 27,281 行=用户看到的 27281；④机制缺陷：启动链每次 loaddata 覆盖数据库，服务器界面改配置重启即还原
