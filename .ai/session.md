...
|------|------|------|----------|------------|--------|--------|
| 第一百六十六轮 | 2026-08-17 | archive、deploy | loaddata事故、容器崩溃、启动链、事故修复、方案A | 165 轮治本（loaddata 条件化）上线后服务器 backend 容器 Restarting 崩溃：日志=loaddata 误判走 else → FieldMapping(pk=11) 撞唯一约束(1,1,2,33)已存在 → 启动链中断 → 前端全挂（假象「域管理东西没了」，数据未丢）；根因=条件命令容器内未实测（验证缺口）+ loaddata 撞唯一约束；用户拍板方案A：启动链彻底移除 loaddata（migrate→init_admin→collectstatic→gunicorn），基线仅首次部署手动导入；commit 99c6c86 已推送；遗留：服务器 pull→up --build 恢复验证 + 数据库残留核查 + 误判根因容器内实测（根因后已确认：条件命令 import 路径错误——INSTALLED_APPS 用 apps. 前缀，from modeling.models 必 ModuleNotFoundError，本机+容器实测复现） | 闸✓记✓拓✓测✓ | 1问/0改向 |
