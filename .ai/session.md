...
## 当前会话状态
- **当前阶段**：第一百六十五轮（服务器 27281 根因定位 + 配置分发治本）——已完成：服务器 diag 输出证实根因（cfg.conditions 空/join_type=left/updated_at=08-11 = data_dump 旧基线；dump 无 join_type 字段致 loaddata 取默认 left；27281=过滤失效时主表匹配行数）；方向锁定（§11.1+adqa 5 条）用户拍板治本；改 docker-compose 启动链 loaddata 条件化 + 重新导出配置基线入库（解除 gitignore）+ 模拟首次部署验证 PASS。遗留：服务器 git pull→重建（loaddata skipped）→DB 补 3 处→diag 重跑→同步收敛验证
- **活跃模块**：archive 同步引擎（诊断+配置分发）
### 最近 3 轮详情（满 3 轮后最旧一轮下沉到详情文件）
- **本次操作**：2026-08-17 — 第一百六十五轮：服务器 27281 根因定位+治本（详见索引行）——根因链：服务器 cfg2/cfg6 conditions=[]+join_type=left+updated_at=08-11（data_dump 旧基线，dump 无 join_type 字段→loaddata 默认 left）；用户本机界面配置只进 dev.db 不随 git 走；27281=kept 209,123 全量→主表匹配 27,281 行；机制缺陷=每次启动 loaddata 覆盖 DB；治本：①compose 启动链 loaddata 条件化（Domain.exists 则跳过，仅首次部署灌）②data_dump.json 重新导出正确基线（排除敏感/业务表，UTF-8）③解除 gitignore 入库；模拟首次部署验证（临时库 migrate+loaddata 133 对象+cfg 正确导入）；服务器待执行 pull→重建→DB 补 3 处（cfg2/cfg6 conditions+join_type、FM9 inner）→同步收敛
- **上次操作**：2026-08-17 — 第一百六十四轮：新增 diag_precombine 诊断命令（详见索引行）——新文件 backend/apps/archive/management/commands/diag_precombine.py（只读）：①配置全景（DetailTableConfig.conditions/join_type/updated_at 原样 + inner 挂载 FieldMapping + cfg.join_type vs fm.join_type 不一致告警）②逐步模拟 inner 挂载过滤（条件拆分→明细行数→头表带条件行数→JOIN 后行数→src_values→same_domain→桥接→kept→交集，与 _build_precombine_filters 逐行对齐）③影子校验（真实函数对比 warnings/结论）④档案统计；本机实跑：挂载#12 价目 239,504→955（header 条件 NAME eq 明码实价 头表 1 行）、挂载#13 分组 64 条（FULL_PARENT_ID starts_with .101041）+桥接物料 209,123→kept 116,594、交集 955、真实函数 0 warnings 生成 row_filter [1,2,6]、档案 active=955 完全吻合；踩坑：Windows GBK 不支持 ✓⚠★→ 符号→ASCII 化；关键证据：本机 cfg2 updated_at=08-17 01:13、cfg6 updated_at=08-13 08:54
- **更早操作**：2026-08-14 — 第一百六十二轮：预组合过滤主记录引擎改造（详见索引行）——7 改动点（_split_conditions 条件拆分 / _build_precombine_filters 预扫 kept 交集 / _upsert_records_from_rows row_filter 行级过滤 / _join_header_rows 头表条件透传 / 异名挂载补洞）；首跑过滤未生效 → 探针定位 target_code 错位 + phys_cols 依赖 code_to_physical 失败；修复：kept 改明细行 source 行内取值 + 主记录侧同域直取/桥接 target_field.table + _sync_detail_rows target_code 标准字段解析 + 挂载键不在 schema 时桥接索引；配置修正 cfg6 FULL_PARENT_ID / cfg2 eq 明码实价（eq 新明码实价 838 物料为 7 位 ID 不在物料表 → 交集 0）；实跑 955 active / 208,168 deactivated / 117,549 details / 0 errors；新测试 10 条 + archive 70/70 PASS；遗留：明细挂 stale 记录、服务器配置暂缓
