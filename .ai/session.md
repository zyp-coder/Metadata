...
## 当前会话状态
- **当前阶段**：第一百七十二轮（测试报告 5 项修复）——问题 1+2 mappings 弹窗加宽、问题 3 api-management 数据预览重设计（ArchiveDetail 风格）、问题 4 archive/3 表格 scroll.y、问题 5 config-tables 编辑弹窗化；全部完成，待部署验证
- **活跃模块**：archive 同步引擎（诊断+配置分发）、deploy
### 最近 3 轮详情（满 3 轮后最旧一轮下沉到详情文件）
- **本次操作**：2026-08-18 — 第一百七十二轮：测试报告 5 项修复——问题 1+2（DomainFieldMapping 4 弹窗加宽）、问题 3（ApiManagement 数据预览重设计：信息栏+字段导航+数据表格）、问题 4（ArchiveDetail scroll.y 加高）、问题 5（ConfigTables 内联编辑→编辑按钮+大弹窗）；vue-tsc 0 + build 7.55s
- **上次操作**：2026-08-18 — 第一百七十一轮：uxqa 全站巡检批4+批5（详见索引行）
- **更早操作**：2026-08-18 — 第一百七十轮：uxqa 全站巡检批3（详见索引行）
...
| 第一百六十八轮 | 2026-08-18 | modeling、uxqa | UI优化、批1巡检、列宽调整 | uxqa 全站巡检批1（DomainList+TableList+ConfigTables）：6项P1+7项P2 全改——配置状态 loading 占位、列宽优化（描述 500→300、操作 320→280 等）、名称链接样式区分、提示文字精简、状态 Switch 统一、删除确认、按钮语义区分；vue-tsc 0 + build 通过 | 闸✓记✓拓✓测✓ | 1问/0改向 |
| 第一百六十七轮 | 2026-08-17 | project | 事故复盘、规则优化、166轮 | 166 轮事故复盘（三层根因链：loaddata 覆盖机制/基线缺 join_type/条件命令 import 错误未容器实测/验证环境不同构）+ 用户确认规则优化落地：R-1 §5 新增③部署链容器实测（自定义命令逐条+实测清单）、R-2 §8 新增「永不放任存量覆盖性机制」、R-3 §7 新增第7条验证环境同构（本机≠容器）、R-4 adqa 时点B新增部署链行为反例检查项；rule 母本/副本已同步 HASH OK | 闸✓记✓拓✓测✓ | 1问/0改向 |
| 第一百六十六轮 | 2026-08-17 | archive、deploy | loaddata事故、容器崩溃、启动链、事故修复、方案A | 165 轮治本（loaddata 条件化）上线后服务器 backend 容器 Restarting 崩溃：日志=loaddata 误判走 else → FieldMapping(pk=11) 撞唯一约束(1,1,2,33)已存在 → 启动链中断 → 前端全挂（假象「域管理东西没了」，数据未丢）；根因=条件命令容器内未实测（验证缺口）+ loaddata 撞唯一约束；用户拍板方案A：启动链彻底移除 loaddata（migrate→init_admin→collectstatic→gunicorn），基线仅首次部署手动导入；commit 99c6c86 已推送；服务器恢复验证全部通过（ps Up/401/counts 1-6-101-7-2 无残留/DB 补 3 处 OK/diag 收敛 955=本机一致）；根因已确认=条件命令 import 路径错误（apps. 前缀，本机+容器实测复现）；用户反馈关系管理多了很多条/语义不一样 → 确认方案后服务器 FM 对齐本机 3 条（保留 FM5/FM11、FM3 改 inner、删 FM4/FM7/FM8/FM9），diag 重跑交集仍 955、影子一致；遗留：界面同步收敛 955 + 关系页 3 条待用户确认 | 闸✓记✓拓✓测✓ | 1问/0改向 |
| 第一百六十五轮 | 2026-08-17 | archive、deploy、project | 27281根因、data_dump基线、loaddata条件化、配置分发、治本、R-001 | 服务器 diag 输出证实根因：cfg.conditions 空/join_type=left/updated_at=08-11（data_dump 旧基线，dump 无 join_type 字段→loaddata 默认 left）；27281=kept 209,123 全量→主表匹配 27,281 行；机制缺陷=每次启动 loaddata 覆盖 DB（用户界面改配置重启即还原）；方向锁定（§11.1+adqa 5 条，#4 存活→R-001）用户拍板治本：①compose 启动链 loaddata 条件化（Domain.exists 则跳过，仅首次部署灌）②data_dump.json 重新导出正确基线（排除敏感/业务表，PYTHONUTF8 防 GBK；模拟首次部署验证 133 对象全量导入+cfg 正确）③解除 gitignore 入库；服务器待执行 pull→重建（loaddata skipped）→DB 补 3 处（cfg2/cfg6 conditions+join_type、FM9 inner）→diag 重跑→同步收敛 | 闸✓记✓拓✓测✓ | 4问/1改向 |
| 第一百六十四轮 | 2026-08-17 | archive | diag_precombine、诊断命令、预组合过滤、本机vs服务器对比 | 新增只读诊断命令 diag_precombine（配置全景/逐步模拟/影子校验/档案统计）：复用同步引擎真实逻辑逐步打印每个 inner 挂载过滤中间量（条件拆分→行数→src_values→same_domain→桥接→kept→交集）；本机实跑：挂载#12 价目 239,504→955（header 条件 NAME eq 明码实价）、挂载#13 分组 64 条限制生效→桥接物料 209,123→kept 116,594、交集 955、影子校验一致（真实函数 0 warnings）、档案 955=955 完全吻合；GBK 符号 ASCII 化兼容；关键证据 cfg2 updated_at=08-17、cfg6 updated_at=08-13；遗留：服务器待跑同命令对比定位 27281 vs 955 根因 | 闸✓记✓拓✓测✓ | 1问/0改向 |
| 第一百六十三轮 | 2026-08-14 | modeling、archive、frontend | 预组合数据预览、detail-configs/{id}/preview、__hdr__、样例匹配优先 | 预组合数据预览功能：DetailTableConfigViewSet preview action（统计 detail_total/detail_hit/header_total/header_matched+样例行，复用同步引擎口径）；archive 引擎最小扩展（count_only 秒级计数+header_rows 复用）；前端管理注册列表+挂载弹窗双入口+预览弹窗（统计卡片+字段/值筛选+limit 切换）；实测暴露物理序前 50 行全未匹配→样例匹配优先修复；新测试 6 条+建模/档案 127/127 PASS+vue-tsc 0；实测价目组合 239,504/239,504/1/955、物料分组 1,782/64/1,782/64 | 闸✓记✓拓✓测✓ | 1问/0改向 |
| 第一百六十二轮 | 2026-08-14 | archive、modeling | 预组合过滤、inner join、筛选条件、kept预扫、row_filter、桥接挂载、FULL_PARENT_ID、eq明码实价 | 预组合过滤引擎改造（方向锁定+adqa 闭环）：7 改动点（_split_conditions / _build_precombine_filters 预扫 kept 交集 / row_filter 行级过滤 / _join_header_rows 头表条件透传 / 异名挂载补洞）；首跑未生效→根因 target_code 错位+phys_cols 依赖 code_to_physical；修复=kept 改 source 行内取值+同域直取/桥接 target_field.table+挂载归属桥接；配置修正（cfg6 FULL_PARENT_ID、cfg2 eq 明码实价）；实跑 209,123→955（deactivated 208,168）+明细挂载 117,549+0 errors；新测试 10 条+archive 70/70 PASS；遗留：明细挂到 stale 记录、服务器配置暂缓 | 闸✓记✓拓✓测✓ | 5问/0改向 |
| 第一百六十一轮 | 2026-08-14 | deploy、建模同步 | 服务器同步超时、HYT00、login timeout、网络放行 | 用户反馈服务器新建档案同步数据报 HYT00 login timeout（4 张 EDS_K3 表）；查证：本机 DataSource=39.108.132.32:1433/METADATA/MD_READ 实测连通（22 表），服务器部署 dump 同配置+Dockerfile 已装 ODBC18（archive.md 旧记录已过时）→ 判定为服务器→SQL Server 网络不通（非 SQLite/Postgres 差异、非配置差异）；已给三步排查清单（TCP 连通测试+公网 IP 获取+安全组/防火墙放行 1433+复验），用户执行中 | N-A | 2问/1改向（用户澄清：非数据源问题，指 Django 主库类型，实际根因为网络） |
