...
## 当前会话状态
- **当前阶段**：第一百六十二轮（预组合过滤主记录引擎改造）——已完成：方向锁定闭环（constitution 决策块+adqa 回执行）；7 改动点实施；首跑过滤未生效 → 根因修复（target_code 标准字段解析 + 预扫 kept 改 source 行内取值 + 同域直取/桥接 target_field.table + 挂载归属桥接）；配置修正（cfg6 FULL_PARENT_ID、cfg2 eq 明码实价）；实跑验证 209,123→955（deactivated 208,168）+ 明细挂载 117,549 + 0 errors；新测试 10 条 + archive 70/70 PASS。遗留：明细挂到 stale 记录（117,549 中约 116,594 属将停用记录）、服务器 data_dump 配置暂缓
- **活跃模块**：archive 同步引擎
### 最近 3 轮详情（满 3 轮后最旧一轮下沉到详情文件）
- **本次操作**：2026-08-14 — 第一百六十二轮：预组合过滤主记录引擎改造（详见索引行）——7 改动点（_split_conditions 条件拆分 / _build_precombine_filters 预扫 kept 交集 / _upsert_records_from_rows row_filter 行级过滤 / _join_header_rows 头表条件透传 / 异名挂载补洞）；首跑过滤未生效 → 探针定位 target_code 错位 + phys_cols 依赖 code_to_physical 失败；修复：kept 改明细行 source 行内取值 + 主记录侧同域直取/桥接 target_field.table + _sync_detail_rows target_code 标准字段解析 + 挂载键不在 schema 时桥接索引；配置修正 cfg6 FULL_PARENT_ID / cfg2 eq 明码实价（eq 新明码实价 838 物料为 7 位 ID 不在物料表 → 交集 0）；实跑 955 active / 208,168 deactivated / 117,549 details / 0 errors；新测试 10 条 + archive 70/70 PASS；遗留：明细挂 stale 记录、服务器配置暂缓
- **上次操作**：2026-08-14 — 第一百六十一轮（服务器网络排查）：用户反馈服务器新建档案同步数据报 HYT00 login timeout（4 张 EDS_K3 表）；已查证：本机 DataSource=39.108.132.32:1433/METADATA/MD_READ 实测连通（22 表）、服务器部署 dump 同配置、Dockerfile 已装 ODBC18 → 判定为服务器→SQL Server 网络不通（排除 SQLite/Postgres 差异）；已给三步排查清单，用户执行中（后续反馈「同步成功了」）
- **更早操作**：2026-08-13 — 第一百五十九轮：用户指出「之前要求过这个界面增加筛选项未加」（留痕漏洞已承认）；批1① handleSubmit reference 清理分支 conditions:null → 按 refConditions 序列化/[]（后端 JSONField 无 null=True 必 400）；批1② 列表卡片顶部搜索框+关系类型下拉（filteredMappings 在 grouping 前过滤，ER 图不受影响）；批2③ 弹窗 reference 分支加行式条件构建器（字段=目标表字段）+ 后端 _upsert_dimension_via_mapping 透传 conditions（仅 reference）；新增 6 测试 + archive 60/60 PASS
