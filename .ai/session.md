...
## 当前会话状态
- **当前阶段**：第一百五十三轮（已完成）——新增 scripts/release.ps1（本地一键发布：build 前端→git add/commit/push）+ deploy/sync.sh（服务器一键同步：git pull→npm run build→重建 backend 自动 migrate→nginx reload）；服务器路径 /opt/metadata/deploy（有 Node/npm）；双脚本语法验证通过
- **活跃模块**：project（运维脚本）
### 最近 3 轮详情（满 3 轮后最旧一轮下沉到详情文件）
- **本次操作**：2026-08-13 — 第一百五十三轮：用户测试工作流诉求「本地一键启动前后端 + 改完保证 commit + 服务器同步」，封装两脚本：① scripts/release.ps1（检查 git 变更→npm run build（失败中止不提交）→-m 参数/交互输入 commit message→git add -A+commit+push origin master，push 前 pre-push hook 自动跑测试，输出服务器下一步提示）；② deploy/sync.sh（git pull→node_modules 缺失自动 npm install→npm run build→docker compose/docker-compose 自动探测→up -d --build backend（启动链自带 migrate，模型变更自动生效）→nginx -s reload 失败兜底 restart）。验证：release.ps1 PowerShell Parser 解析 OK + sync.sh git-bash bash -n OK
