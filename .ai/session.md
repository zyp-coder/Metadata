...
## 当前会话状态
- **当前阶段**：第一百六十四轮（新增 diag_precombine 诊断命令）——已完成：prjm 路由+用户确认方案；新建只读诊断命令 diag_precombine（配置全景/逐步模拟/影子校验/档案统计四部分，复用同步引擎真实逻辑）；本机实跑验证通过（域 2：挂载#12 价目 239,504→955、挂载#13 分组 64 条限制生效、交集 955、影子校验一致、档案 955=955）；GBK 符号兼容修复。遗留：服务器待跑同命令对比（重点 cfg.conditions/cfg.updated_at）定位 27281 vs 955 根因
- **活跃模块**：archive 同步引擎（诊断）
### 最近 3 轮详情（满 3 轮后最旧一轮下沉到详情文件）
- **本次操作**：2026-08-17 — 第一百六十四轮：新增 diag_precombine 诊断命令（详见索引行）——新文件 backend/apps/archive/management/commands/diag_precombine.py（只读）：①配置全景（DetailTableConfig.conditions/join_type/updated_at 原样 + inner 挂载 FieldMapping + cfg.join_type vs fm.join_type 不一致告警）②逐步模拟 inner 挂载过滤（条件拆分→明细行数→头表带条件行数→JOIN 后行数→src_values→same_domain→桥接→kept→交集，与 _build_precombine_filters 逐行对齐）③影子校验（真实函数对比 warnings/结论）④档案统计；本机实跑：挂载#12 价目 239,504→955（header 条件 NAME eq 明码实价 头表 1 行）、挂载#13 分组 64 条（FULL_PARENT_ID starts_with .101041）+桥接物料 209,123→kept 116,594、交集 955、真实函数 0 warnings 生成 row_filter [1,2,6]、档案 active=955 完全吻合；踩坑：Windows GBK 不支持 ✓⚠★→ 符号→ASCII 化；关键证据：本机 cfg2 updated_at=08-17 01:13、cfg6 updated_at=08-13 08:54
- **上次操作**：2026-08-14 — 第一百六十二轮：预组合过滤主记录引擎改造（详见索引行）——7 改动点（_split_conditions 条件拆分 / _build_precombine_filters 预扫 kept 交集 / _upsert_records_from_rows row_filter 行级过滤 / _join_header_rows 头表条件透传 / 异名挂载补洞）；首跑过滤未生效 → 探针定位 target_code 错位 + phys_cols 依赖 code_to_physical 失败；修复：kept 改明细行 source 行内取值 + 主记录侧同域直取/桥接 target_field.table + _sync_detail_rows target_code 标准字段解析 + 挂载键不在 schema 时桥接索引；配置修正 cfg6 FULL_PARENT_ID / cfg2 eq 明码实价（eq 新明码实价 838 物料为 7 位 ID 不在物料表 → 交集 0）；实跑 955 active / 208,168 deactivated / 117,549 details / 0 errors；新测试 10 条 + archive 70/70 PASS；遗留：明细挂 stale 记录、服务器配置暂缓
- **更早操作**：2026-08-14 — 第一百六十一轮（服务器网络排查）：用户反馈服务器新建档案同步数据报 HYT00 login timeout（4 张 EDS_K3 表）；已查证：本机 DataSource=39.108.132.32:1433/METADATA/MD_READ 实测连通（22 表）、服务器部署 dump 同配置、Dockerfile 已装 ODBC18 → 判定为服务器→SQL Server 网络不通（排除 SQLite/Postgres 差异）；已给三步排查清单，用户执行中（后续反馈「同步成功了」）
