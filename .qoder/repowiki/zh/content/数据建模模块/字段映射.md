...
- 更新了DomainFieldMapping.vue界面的重大重构（三面板布局设计）
- 添加了条件构建器的字段来源切换功能
- 完善了ER图的预组合关系可视化展示
- 扩展了字段映射的JOIN类型配置能力
- **新增**：左右分栏的三面板布局设计，提供更直观的预组合关系配置体验
...
**更新** 本次更新重点增强了字段映射的预组合能力，支持更复杂的多表关联场景，包括头表与明细表的预组合、JOIN类型配置以及改进的用户界面体验。特别是新增了左右分栏的三面板布局设计，将传统的下拉选择表单重构为更直观的预组合关系配置体验。
...
**更新** 本次增强显著提升了字段映射的预组合能力，支持更复杂的多表关联场景，特别是头表+明细表的组合模式，为价格表、分组表等业务场景提供了更好的支持。**三面板布局和预组合可视化展示大幅改善了用户体验**，通过左右分栏的设计让用户能够更直观地配置头表与明细表的关联关系。
