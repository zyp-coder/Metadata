...
## 核心组件
- 后端模型层
  - modeling：DataSource、Domain、Table、FieldGroup、StandardField、ComputedField 等
...
- 后端测试套件
  ...
  - apps/archive/tests.py：模型导入、URL 解析、记录访问、版本追踪、变更日志、API 模型
- 后端集成测试
...
**章节来源**
- [backend/apps/modeling/tests.py:1-212](file://backend/apps/modeling/tests.py#L1-L212)
- [backend/apps/archive/tests.py:1-231](file://backend/apps/archive/tests.py#L1-L231)
- [backend/test_api.py:1-155](file://backend/test_api.py#L1-L155)
...
## 结论
本项目已具备完善的后端单元测试与 API 集成测试基础，覆盖了建模与档案两大核心领域的关键流程。前端测试尚未落地，建议尽快引入现代前端测试框架以保障 UI 与状态管理的稳定性。通过统一脚本与清晰的测试分层，可在持续集成中实现高效的自动化质量门禁。
