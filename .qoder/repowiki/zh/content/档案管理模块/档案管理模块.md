...
## 更新摘要
**变更内容**   
- 档案同步系统重大架构改进：支持通过任意字段建立一对多关系，`_sync_detail_rows` 方法完全重构以支持非主键挂载点
- 代表行写入逻辑更新为处理一对多场景，按挂载字段值分组处理，每组排序首行作为默认值写入主表
- 前端超时处理增强：`refreshPreview` API 超时时间从 30s 增加到 180s，支持大数据量预检场景
- 用户反馈机制完善：增加同步状态显示和友好的超时错误提示，提升用户体验
...
**最新更新**：档案同步系统经过重大架构改进，支持通过任意字段建立一对多关系，显著提升了复杂业务场景下的数据关联处理能力，包括代表行处理逻辑优化、前端超时处理增强和用户反馈机制完善。
...
- [backend/apps/archive/views.py:1-3919](file://backend/apps/archive/views.py#L1-L3919)
...
**最新更新**：经过重大架构改进的档案同步系统，特别是支持通过任意字段建立一对多关系的能力，显著提升了复杂业务场景下的数据处理能力。新的非主键挂载点机制、优化的代表行处理逻辑、增强的前端超时处理和完善的用户反馈机制，共同确保了系统在大规模数据场景下的稳定性和可靠性。
