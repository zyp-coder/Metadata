...
## 更新摘要
**变更内容**   
- 档案同步系统重大改进：代表行处理逻辑修复、前端超时处理增强、用户反馈机制完善
- `_sync_detail_rows` 方法实现按物料分组处理代表行，确保每组排序首行作为默认值写入主表
- `refreshPreview` API 超时时间从 30s 增加到 180s，支持大数据量预检场景
- ArchiveList 组件增加同步状态显示和友好的超时错误提示，提升用户体验
...
**最新更新**：档案同步系统经过重大改进，包括代表行处理逻辑优化、前端超时处理增强和用户反馈机制完善，显著提升了大数据量场景下的稳定性和用户体验。
...
- [backend/apps/archive/views.py:1-1200](file://backend/apps/archive/views.py#L1-L1200)
...
### 档案与记录：双层存储与合并策略
- 双层存储：source_data 为底层（同步时整层替换），manual_data 为人工覆盖层（仅允许 ownership=archive 的字段有键）。
...
**新增**：代表行处理逻辑经过重大改进，实现了按物料分组的智能处理机制。

- **分组处理**：`_sync_detail_rows` 方法现在按物料分组处理代表行，每组选择排序首行作为默认值写入主表。
- **排序算法**：使用 display_sort_field 进行排序（空值垫底），结合行键次级键确保确定性。
- **语义对齐**：对齐第133轮方向锁定语义，确保默认价等业务逻辑的正确性。
- **性能优化**：通过分组处理减少重复计算，提升大数据量场景下的处理效率。

```mermaid
flowchart TD
Start(["开始代表行处理"]) --> GroupByMaterial["按物料分组"]
GroupByMaterial --> SortRows["组内排序<br/>display_sort_field DESC + 行键次级键"]
SortRows --> SelectFirst["选择每组首行作为代表行"]
SelectFirst --> WriteMainTable["写入主表<br/>source_data 合并 + 版本+1"]
WriteMainTable --> ChangeLog["记录变更日志"]
ChangeLog --> End(["处理完成"])
```

**图表来源** 
- [backend/apps/archive/views.py:2081-2096](file://backend/apps/archive/views.py#L2081-L2096)

**章节来源**
- [backend/apps/archive/views.py:1835-2233](file://backend/apps/archive/views.py#L1835-L2233)
...
- **代表行分组处理**：按物料分组处理代表行，减少重复计算，提升大数据量场景下的处理效率。
...
- **代表行异常**：检查 display_sort_field 配置是否正确，确认物料分组逻辑是否符合业务预期。
...
**最新更新**：经过重大改进的档案同步系统，包括代表行处理逻辑优化、前端超时处理增强和用户反馈机制完善，显著提升了大数据量场景下的稳定性和用户体验。这些改进确保了系统在处理大规模数据时的可靠性和响应性。
...
- **代表行配置**：
  - 正确配置 display_sort_field 以确保代表行选择的准确性。
  - 验证物料分组逻辑是否符合业务需求，确保默认值选择的正确性。

**章节来源**
- [backend/apps/archive/views.py:47-159](file://backend/apps/archive/views.py#L47-L159)
- [backend/apps/archive/views.py:800-928](file://backend/apps/archive/views.py#L800-L928)
- [frontend/src/views/archive/ArchiveList.vue:262-315](file://frontend/src/views/archive/ArchiveList.vue#L262-L315)
- [frontend/src/views/archive/ConsistencyCheck.vue:335-351](file://frontend/src/views/archive/ConsistencyCheck.vue#L335-L351)
- [frontend/src/views/archive/VersionManagement.vue:547-583](file://frontend/src/views/archive/VersionManagement.vue#L547-L583)