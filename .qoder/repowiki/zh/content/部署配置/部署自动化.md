...
## 更新摘要
**变更内容**
- 更新了服务器一键同步脚本（sync.sh）的详细说明，反映其从50+行简化为10行的重大变更
- 移除了复杂错误处理和路径检测逻辑的描述
- 更新了同步流程图表以反映简化的执行逻辑
- 调整了故障排查指南中关于同步脚本的使用说明
...
### 服务器一键同步（sync.sh）
**已更新** 同步脚本已从50+行大幅简化为10行，移除了复杂的错误处理和路径检测逻辑，专注于核心功能。

- **简化设计**：脚本假设在 `/opt/metadata` 目录下执行，移除了路径检测和兼容性处理。
- **核心步骤**：git pull -> npm build -> docker compose up --build backend -> nginx reload。
- **简洁可靠**：移除复杂错误处理后，脚本更加简洁且易于维护。

```mermaid
flowchart TD
Start(["开始"]) --> CD["cd /opt/metadata"]
CD --> Pull["git pull origin master"]
Pull --> BuildFE["cd frontend && npm run build"]
BuildFE --> RebuildBE["cd ../deploy && docker compose up -d --build backend"]
RebuildBE --> ReloadNGINX["docker compose exec nginx nginx -s reload"]
ReloadNGINX --> End(["结束"])
```
...
- 发布后未生效：
  - **重要**：确认服务器在 `/opt/metadata` 目录下执行了 `bash /opt/metadata/deploy/sync.sh`。
  - 由于简化后的 sync.sh 不再包含路径检测，必须在正确的目录下执行。
  - 必要时手动执行 `docker compose exec nginx nginx -s reload` 重新加载配置。
...
- **简化的运维**：sync.sh 脚本的大幅简化降低了维护复杂度，提高了可靠性。
...
- 常用命令
  ...
  - **服务器同步**：`bash /opt/metadata/deploy/sync.sh`（必须在 `/opt/metadata` 目录下执行）

**章节来源**
- [deploy/sync.sh:1-10](file://deploy/sync.sh#L1-L10)
- [scripts/release.ps1:60-64](file://scripts/release.ps1#L60-L64)