...
- [backend/scripts/check_db_diagnostics.py](file://backend/scripts/check_db_diagnostics.py)
</cite>
...
- **新增部署有效性诊断方法论**：基于第158轮实战案例的系统化故障排查流程，涵盖进程验证、构建产物校验和服务配置检查
...
基于第158轮的真实部署故障场景，建立了完整的部署有效性诊断方法论。该案例展示了当用户反馈功能异常时，如何通过系统化的方法定位问题根源。

#### 典型问题场景
用户反馈「编辑功能无法正常工作」，经过深入诊断发现：
- 后端进程使用 `--noreload` 标志运行旧版本代码
- 前端 dist 构建产物版本过旧（8月8日构建 vs 8月13日的代码修改）
- 本地无前端开发服务，直接访问服务器部署的旧版本
...
ps aux | grep python
# 验证是否使用 --noreload 标志
ps aux | grep "manage.py.*--noreload"
# 检查进程启动时间
ps -p <PID> -o lstart=
```
...
# 对比代码修改时间与构建时间
git log --oneline -5
find frontend/dist -type f -exec stat -c '%y %n' {} \;
```

**诊断要点**：
- 确认 dist 构建时间晚于代码修改时间
- 验证构建脚本是否正确执行
- 检查是否有缓存导致的旧文件残留
...
##### 2. 监控与告警
- 部署时间戳监控：记录每次部署的时间点和版本信息
- 构建产物完整性检查：验证关键文件的 MD5 校验和
- 服务可用性监控：定期检查关键端点的响应状态
...
| 问题现象 | 可能原因 | 解决方案 |
|---------|----------|----------|
| 功能不生效 | 后端使用 --noreload | 重启后端服务 |
| 前端样式错乱 | dist 构建过时 | 重新执行 npm run build |
| 404 错误 | Nginx 配置错误 | 检查静态文件路径 |
| 数据库连接失败 | 环境变量配置错误 | 检查 DB_* 环境变量 |
| 缓存失效 | Redis 连接问题 | 检查 REDIS_* 配置 |

#### 最佳实践

1. **版本锁定**：使用 Git Tag 标记稳定版本，部署时指定具体版本号
2. **灰度发布**：先部署到测试环境验证，再逐步推广到生产环境
3. **日志集中**：统一收集各组件日志，便于问题追踪
4. **监控完善**：建立完善的监控体系，及时发现异常情况
5. **文档更新**：每次部署后更新部署文档和变更记录

**章节来源**
- [deploy/sync.sh:1-10](file://deploy/sync.sh#L1-L10)
- [scripts/release.ps1:1-69](file://scripts/release.ps1#L1-L69)
- [backend/scripts/check_db_diagnostics.py:1-331](file://backend/scripts/check_db_diagnostics.py#L1-L331)

## 结论
MetaData002 采用成熟的 Django + Vue 技术栈，具备清晰的容器化与编排能力。生产部署应重点落实环境变量与密钥管理、Nginx 反向代理与 SSL、监控与日志体系、备份与恢复演练、性能与安全加固。通过第158轮实战案例建立的部署有效性诊断方法论，可以系统化地解决部署过程中的各类问题，确保生产环境的稳定性和可靠性。遵循本文档建议，可实现稳定、可扩展、易维护的生产环境。
...
- Alibaba Cloud Linux 3 注意事项
  ...
  - 使用 .env 文件管理敏感配置，禁止提交到代码库
- 部署脚本使用指南
  - 本地发布：`.\scripts\release.ps1 [-m "commit message"]`
  - 服务器同步：`bash /opt/metadata/deploy/sync.sh`
- 诊断工具使用
  - 数据库诊断：`python backend/scripts/check_db_diagnostics.py`
  - 版本检查：使用上述提供的版本检查脚本
  - 构建验证：使用构建验证脚本确保构建质量