# 字段映射模型

<cite>
**本文引用的文件**   
- [models.py](file://backend/apps/modeling/models.py)
- [serializers.py](file://backend/apps/modeling/serializers.py)
- [views.py](file://backend/apps/modeling/views.py)
- [urls.py](file://backend/apps/modeling/urls.py)
- [ai_service.py](file://backend/apps/modeling/ai_service.py)
- [DomainFieldMapping.vue](file://frontend/src/views/modeling/DomainFieldMapping.vue)
- [modeling.ts](file://frontend/src/api/modeling.ts)
</cite>

## 更新摘要
**变更内容**   
- 新增关系类型（relation_type）支持普通关联和明细子表关系
- 增强筛选条件（conditions）功能，支持结构化 ON/WHERE 条件配置
- 新增连接类型（join_type）支持 LEFT JOIN 和 INNER JOIN
- 改进前端交互，支持关系类型过滤和条件可视化编辑
- 优化预组合子表注册机制，支持头表+明细表的预组合体

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 引言
本文件围绕 MetaData002 系统的"字段映射模型"进行系统化文档化，重点解释 FieldMapping 模型类的设计与使用，包括表间关系的定义、字段级映射规则、双向关联管理、CRUD 操作、唯一性约束与完整性校验、查询优化、批量操作与一致性检查机制，并给出最佳实践与复杂关系处理方案。该模型用于在多表域中通过"源表.源字段 → 目标表.目标字段"的映射表达表间关系，支撑档案合并、数据同步与一致性校验等上层能力。

**更新** 本次更新增强了字段映射功能，新增了关系类型区分、高级筛选条件和连接类型控制，以及改进的前端交互体验。

## 项目结构
字段映射功能涉及后端模型、序列化器、视图集、路由以及前端交互与 AI 推断服务：
- 模型层：FieldMapping 及其相关实体（Table、Field、DetailTableConfig）
- 序列化层：FieldMappingSerializer、DetailTableConfigSerializer
- API 层：FieldMappingViewSet（含 infer-mappings 自定义动作）
- 路由层：field-mappings 路由注册
- 前端：DomainFieldMapping.vue 页面与 modeling.ts API 封装
- AI 服务：ai_service.infer_mappings 提供智能建议

```mermaid
graph TB
subgraph "前端"
FE_VM["DomainFieldMapping.vue"]
FE_API["modeling.ts"]
end
subgraph "后端API"
URLS["urls.py"]
VIEWS["views.py<br/>FieldMappingViewSet"]
SER["serializers.py<br/>FieldMappingSerializer"]
end
subgraph "领域模型"
MODELS["models.py<br/>FieldMapping/Table/Field/DetailTableConfig"]
AI["ai_service.py<br/>infer_mappings()"]
end
FE_VM --> FE_API
FE_API --> URLS
URLS --> VIEWS
VIEWS --> SER
VIEWS --> MODELS
VIEWS --> AI
```

图表来源
- [urls.py:12](file://backend/apps/modeling/urls.py#L12)
- [views.py:1824-1879](file://backend/apps/modeling/views.py#L1824-L1879)
- [serializers.py:337-396](file://backend/apps/modeling/serializers.py#L337-L396)
- [models.py:519-575](file://backend/apps/modeling/models.py#L519-L575)
- [ai_service.py:754-836](file://backend/apps/modeling/ai_service.py#L754-L836)
- [DomainFieldMapping.vue:711-738](file://frontend/src/views/modeling/DomainFieldMapping.vue#L711-L738)
- [modeling.ts:467-480](file://frontend/src/api/modeling.ts#L467-L480)

章节来源
- [urls.py:12](file://backend/apps/modeling/urls.py#L12)
- [views.py:1824-1879](file://backend/apps/modeling/views.py#L1824-L1879)
- [serializers.py:337-396](file://backend/apps/modeling/serializers.py#L337-L396)
- [models.py:519-575](file://backend/apps/modeling/models.py#L519-L575)
- [ai_service.py:754-836](file://backend/apps/modeling/ai_service.py#L754-L836)
- [DomainFieldMapping.vue:711-738](file://frontend/src/views/modeling/DomainFieldMapping.vue#L711-L738)
- [modeling.ts:467-480](file://frontend/src/api/modeling.ts#L467-L480)

## 核心组件
- FieldMapping 模型：定义"源表.源字段 → 目标表.目标字段"的映射关系，包含时间戳与复合唯一约束，支持关系类型、连接类型和筛选条件。
- Table/Field 模型：作为映射的两端实体，分别承载表与字段的元数据。
- DetailTableConfig 模型：预组合子表注册，支持头表+明细表的组合体配置。
- FieldMappingSerializer：序列化/反序列化映射记录，附带可读名称字段和关系类型标签。
- FieldMappingViewSet：提供标准 CRUD 与按表/域过滤，以及 infer-mappings 智能推断接口。
- ai_service.infer_mappings：基于 LLM 或启发式算法生成映射建议，避免重复。

章节来源
- [models.py:519-575](file://backend/apps/modeling/models.py#L519-L575)
- [models.py:60-123](file://backend/apps/modeling/models.py#L60-L123)
- [models.py:301-367](file://backend/apps/modeling/models.py#L301-L367)
- [models.py:577-625](file://backend/apps/modeling/models.py#L577-L625)
- [serializers.py:337-396](file://backend/apps/modeling/serializers.py#L337-L396)
- [views.py:1824-1879](file://backend/apps/modeling/views.py#L1824-L1879)
- [ai_service.py:754-836](file://backend/apps/modeling/ai_service.py#L754-L836)

## 架构总览
字段映射的整体调用链路如下：
- 前端在 DomainFieldMapping.vue 发起"AI 自动映射"请求，调用 modeling.ts 封装的 inferMappings。
- 后端 urls.py 将 /field-mappings/infer-mappings/ 路由到 FieldMappingViewSet.infer_mappings。
- 视图调用 ai_service.infer_mappings 获取建议，补充表名/字段名后返回。
- 用户确认后，前端循环调用 create 创建映射；若涉及组合主键，会批量创建多条映射。

**更新** 新增的关系类型和筛选条件在前端界面中提供了更直观的可视化编辑体验。

```mermaid
sequenceDiagram
participant FE as "前端页面<br/>DomainFieldMapping.vue"
participant API as "API封装<br/>modeling.ts"
participant Router as "路由<br/>urls.py"
participant View as "视图集<br/>FieldMappingViewSet"
participant AI as "AI服务<br/>ai_service.infer_mappings"
participant DB as "数据库"
FE->>API : 调用 inferMappings(domainId)
API->>Router : POST /field-mappings/infer-mappings/
Router->>View : 分发到 infer_mappings
View->>AI : 传入 domainId，获取建议
AI-->>View : 返回 suggestions含置信度/原因
View-->>API : 返回 suggestions填充表/字段名
API-->>FE : 展示建议列表
FE->>API : 选择建议后批量 create(...)
API->>Router : POST /field-mappings/
Router->>View : 创建映射
View->>DB : 写入 FieldMapping受唯一约束保护
DB-->>View : 成功/失败
View-->>API : 返回结果
API-->>FE : 更新界面
```

图表来源
- [DomainFieldMapping.vue:711-738](file://frontend/src/views/modeling/DomainFieldMapping.vue#L711-L738)
- [modeling.ts:467-480](file://frontend/src/api/modeling.ts#L467-L480)
- [urls.py:12](file://backend/apps/modeling/urls.py#L12)
- [views.py:1824-1879](file://backend/apps/modeling/views.py#L1824-L1879)
- [ai_service.py:754-836](file://backend/apps/modeling/ai_service.py#L754-L836)

## 详细组件分析

### FieldMapping 模型设计
- 字段说明
  - source_table/target_table：外键指向 Table，表示映射的源/目标表。
  - source_field/target_field：外键指向 Field，表示映射的源/目标字段。
  - relation_type：关系类型，区分普通关联（reference）和明细子表关系（detail）。
  - join_type：连接类型，支持 LEFT JOIN 和 INNER JOIN。
  - conditions：结构化筛选条件，支持 AND 组合的条件表达式。
  - created_at：创建时间。
- 唯一性约束
  - unique_together = (source_table, source_field, target_table, target_field)，确保同一对源/目标字段映射唯一。
- 反向关联
  - source_mappings：Table → FieldMapping（作为源表）
  - target_mappings：Table → FieldMapping（作为目标表）
  - Field 侧也有 source_mappings/target_mappings 反向访问。
- 业务语义
  - 通过字段级映射表达表间关系，支持一对一、一对多、多对一、多对多（由多个映射组合实现）。
  - 结合 StandardField 概念层去重与主字段策略，可驱动档案合并与一致性校验。

**更新** 新增的关系类型支持普通关联和明细子表关系，连接类型控制数据保留策略，筛选条件支持灵活的 ON/WHERE 条件配置。

```mermaid
classDiagram
class Table {
+id
+name
+code
+is_primary
+status
}
class Field {
+id
+name
+code
+is_primary_key
+table_id
+status
}
class FieldMapping {
+id
+source_table_id
+source_field_id
+target_table_id
+target_field_id
+relation_type
+join_type
+conditions
+created_at
}
class DetailTableConfig {
+id
+domain_id
+table_id
+header_table_id
+header_link_field_id
+detail_link_field_id
+row_key_field_id
+display_sort_field_id
+display_sort_desc
+join_type
+conditions
+created_at
+updated_at
}
Table "1" --> "many" Field : "拥有字段"
Table "1" --> "many" FieldMapping : "source_mappings"
Table "1" --> "many" FieldMapping : "target_mappings"
Field "1" --> "many" FieldMapping : "source_mappings"
Field "1" --> "many" FieldMapping : "target_mappings"
DetailTableConfig "1" --> "many" FieldMapping : "mappings"
```

图表来源
- [models.py:60-123](file://backend/apps/modeling/models.py#L60-L123)
- [models.py:301-367](file://backend/apps/modeling/models.py#L301-L367)
- [models.py:519-575](file://backend/apps/modeling/models.py#L519-L575)
- [models.py:577-625](file://backend/apps/modeling/models.py#L577-L625)

章节来源
- [models.py:519-575](file://backend/apps/modeling/models.py#L519-L575)

### 字段映射的 CRUD 与查询
- 标准 CRUD
  - 列表：支持按 source_table/target_table 过滤，支持按 table/domain 参数筛选（get_queryset 扩展）。
  - 详情/创建/更新/删除：由 ModelViewSet 默认提供。
- 序列化
  - FieldMappingSerializer 暴露 id、source/target 表与字段 ID，以及对应的 name 只读字段，便于前端展示。
  - 新增 relation_type_label、join_type_label 等显示字段。
- 过滤与检索
  - filterset_fields 支持 source_table/target_table。
  - get_queryset 支持 table 与 domain 查询参数，提升前端列表性能。

**更新** 序列化器现在包含关系类型和连接类型的显示标签，支持更丰富的前端展示需求。

章节来源
- [views.py:1824-1843](file://backend/apps/modeling/views.py#L1824-L1843)
- [serializers.py:337-396](file://backend/apps/modeling/serializers.py#L337-L396)

### 智能推断映射（AI 辅助）
- 入口
  - FieldMappingViewSet.infer_mappings：接收 domainId，调用 ai_service.infer_mappings。
- 逻辑
  - 收集域内活跃表与字段，读取已存在映射避免重复建议。
  - 优先 LLM 智能分析，失败回退到启发式匹配（编码相同且非双主键）。
  - 返回 suggestions（source/target 字段ID、置信度、原因），视图层补充表名/字段名。
- 前端交互
  - DomainFieldMapping.vue 调用 inferMappings，展示建议并允许批量确认创建。
  - 针对组合主键场景，前端会按最小主键数批量创建映射。

```mermaid
flowchart TD
Start(["开始"]) --> GetDomain["获取域ID"]
GetDomain --> CollectTables["收集域内活跃表与字段"]
CollectTables --> LoadExisting["加载已存在的映射(去重)"]
LoadExisting --> TryLLM{"是否配置LLM?"}
TryLLM --> |是| CallLLM["调用LLM分析映射"]
TryLLM --> |否| Heuristic["启发式匹配(编码相同且非双主键)"]
CallLLM --> Merge["合并建议(去重)"]
Heuristic --> Merge
Merge --> Enrich["补充表名/字段名"]
Enrich --> Return["返回 suggestions"]
Return --> End(["结束"])
```

图表来源
- [views.py:1845-1879](file://backend/apps/modeling/views.py#L1845-L1879)
- [ai_service.py:754-836](file://backend/apps/modeling/ai_service.py#L754-L836)
- [DomainFieldMapping.vue:711-738](file://frontend/src/views/modeling/DomainFieldMapping.vue#L711-L738)

章节来源
- [views.py:1845-1879](file://backend/apps/modeling/views.py#L1845-L1879)
- [ai_service.py:754-836](file://backend/apps/modeling/ai_service.py#L754-L836)
- [DomainFieldMapping.vue:711-738](file://frontend/src/views/modeling/DomainFieldMapping.vue#L711-L738)

### 关系类型与筛选条件
- 关系类型
  - reference：普通关联，用于属性展开/折叠。
  - detail：明细子表关系，目标表作为 source_table 的明细致子表，同步保留全部行。
- 连接类型
  - left：LEFT JOIN，保留无匹配行。
  - inner：INNER JOIN，仅保留匹配行。
- 筛选条件
  - conditions：JSON 格式的结构化条件数组，支持 AND 组合。
  - 条件格式：[{"field": "物理列名或字段编码", "operator": "eq/ne/gt/ge/lt/le/in", "value": 值}]
- 预组合子表
  - DetailTableConfig 支持头表+明细表的预组合体配置。
  - 同一 detail_config 可被多个映射挂载（一子表多主表）。

**更新** 新增的关系类型和筛选条件为复杂的表间关系提供了更精细的控制能力。

章节来源
- [models.py:519-575](file://backend/apps/modeling/models.py#L519-L575)
- [models.py:577-625](file://backend/apps/modeling/models.py#L577-L625)
- [serializers.py:257-302](file://backend/apps/modeling/serializers.py#L257-L302)

### 唯一性约束与关系完整性验证
- 唯一性
  - FieldMapping.unique_together(source_table, source_field, target_table, target_field) 保证映射唯一。
- 完整性
  - 停用表前检查是否存在映射关系（toggle_status 动作），防止破坏关系。
  - 域配置检查（_check_domain_config）提示多表域缺少映射的风险。
- 一致性
  - 档案一致性检查利用映射将物理列映射到 schema code，进行差异比对。

**更新** 新增的唯一性验证器提供更友好的错误信息，明确指出冲突的映射关系。

章节来源
- [models.py:519-575](file://backend/apps/modeling/models.py#L519-L575)
- [views.py:690-721](file://backend/apps/modeling/views.py#L690-L721)
- [views.py:327-427](file://backend/apps/modeling/views.py#L327-L427)
- [archive/views.py:673-702](file://backend/apps/archive/views.py#L673-L702)

### 批量操作与一致性检查
- 批量创建
  - 前端根据 AI 建议或组合主键情况，循环调用 create，后端逐条落库并由唯一约束保障一致性。
- 一致性检查
  - 域配置检查输出 P0/P1/P2 级别问题清单，其中 P2 提示多表域未配置映射。
  - 档案一致性检查通过映射将源表字段映射到 schema code，对比档案与源数据差异。

**更新** 前端现在支持关系类型过滤和条件可视化编辑，提升了批量操作的效率。

章节来源
- [DomainFieldMapping.vue:850-1049](file://frontend/src/views/modeling/DomainFieldMapping.vue#L850-L1049)
- [views.py:327-427](file://backend/apps/modeling/views.py#L327-L427)
- [archive/views.py:673-702](file://backend/apps/archive/views.py#L673-L702)

## 依赖关系分析
- 模型依赖
  - FieldMapping 依赖 Table 与 Field，形成强一致的外键关系。
  - DetailTableConfig 依赖 Domain 和 Table，支持预组合体的配置。
- 视图依赖
  - FieldMappingViewSet 依赖 FieldMappingSerializer 与 models，并在 infer_mappings 中调用 ai_service。
- 前端依赖
  - DomainFieldMapping.vue 依赖 modeling.ts 提供的 API 封装，调用 inferMappings 与 create/delete。

**更新** 新增的 DetailTableConfig 模型为预组合子表提供了独立的配置管理能力。

```mermaid
graph LR
Models["models.py<br/>FieldMapping/Table/Field/DetailTableConfig"] --> Views["views.py<br/>FieldMappingViewSet"]
Serializers["serializers.py<br/>FieldMappingSerializer"] --> Views
AI["ai_service.py<br/>infer_mappings"] --> Views
Frontend["DomainFieldMapping.vue"] --> API["modeling.ts"]
API --> Views
```

图表来源
- [models.py:519-575](file://backend/apps/modeling/models.py#L519-L575)
- [models.py:577-625](file://backend/apps/modeling/models.py#L577-L625)
- [serializers.py:337-396](file://backend/apps/modeling/serializers.py#L337-L396)
- [views.py:1824-1879](file://backend/apps/modeling/views.py#L1824-L1879)
- [ai_service.py:754-836](file://backend/apps/modeling/ai_service.py#L754-L836)
- [DomainFieldMapping.vue:711-738](file://frontend/src/views/modeling/DomainFieldMapping.vue#L711-L738)
- [modeling.ts:467-480](file://frontend/src/api/modeling.ts#L467-L480)

章节来源
- [models.py:519-575](file://backend/apps/modeling/models.py#L519-L575)
- [models.py:577-625](file://backend/apps/modeling/models.py#L577-L625)
- [serializers.py:337-396](file://backend/apps/modeling/serializers.py#L337-L396)
- [views.py:1824-1879](file://backend/apps/modeling/views.py#L1824-L1879)
- [ai_service.py:754-836](file://backend/apps/modeling/ai_service.py#L754-L836)
- [DomainFieldMapping.vue:711-738](file://frontend/src/views/modeling/DomainFieldMapping.vue#L711-L738)
- [modeling.ts:467-480](file://frontend/src/api/modeling.ts#L467-L480)

## 性能考虑
- 查询优化
  - 使用 select_related('source_table','source_field','target_table','target_field') 减少 N+1 查询。
  - 列表过滤支持 table/domain 参数，缩小结果集。
- 批量操作
  - 前端批量创建映射时注意网络往返开销，可在服务端增加批量接口（当前为逐条创建）。
- 缓存与样本值
  - 字段 distinct_values 缓存可用于 AI 推断与一致性检查，降低实时采样成本。

**更新** 新增的关系类型和筛选条件在查询时需要额外的索引优化，建议在高频查询字段上建立索引。

## 故障排查指南
- 常见错误
  - 唯一性冲突：创建映射时报 400，检查是否已存在相同映射。
  - 停用表失败：表存在映射关系时不允许停用，需先解除映射。
  - AI 推断失败：未配置 API Key 或网络异常，回退到启发式匹配。
  - 关系类型错误：detail 类型必须配置 detail_config 和 target_field。
- 定位步骤
  - 查看 FieldMapping 列表，确认映射是否存在。
  - 检查域配置检查结果（P0/P1/P2），关注"多表已配置关系映射"项。
  - 核对表的主键配置与状态，确保映射两端有效。
  - 检查 conditions JSON 格式是否正确，避免 null 值。

**更新** 新增的错误类型包括关系类型验证错误和条件格式错误，需要特别注意。

章节来源
- [views.py:690-721](file://backend/apps/modeling/views.py#L690-L721)
- [views.py:327-427](file://backend/apps/modeling/views.py#L327-L427)
- [ai_service.py:754-836](file://backend/apps/modeling/ai_service.py#L754-L836)

## 结论
FieldMapping 模型以简洁的字段级映射表达表间关系，配合唯一性约束与视图层的过滤/推断能力，为多表域的数据治理提供了坚实基础。通过 AI 辅助推断与前端批量操作，显著降低人工配置成本；结合域配置检查与档案一致性校验，保障关系完整性与数据一致性。

**更新** 本次更新增强了字段映射的功能性，通过关系类型区分、连接类型控制和筛选条件支持，为复杂的表间关系提供了更精细的管理能力。建议在复杂关系中优先使用主键映射，并结合 StandardField 主字段策略，确保档案合并与同步的正确性与可维护性。

## 附录
- 最佳实践
  - 优先为主表主键建立映射，明确数据源头。
  - 对组合主键场景，按最小主键数批量创建映射，避免遗漏。
  - 定期运行域配置检查，及时修复缺失映射与主键配置。
  - 合理使用关系类型：普通关联用于简单引用，明细子表用于复杂的多行数据。
  - 谨慎使用筛选条件，避免过度复杂的条件表达式影响性能。
- 复杂关系处理
  - 多对多关系通过多条映射组合表达，注意唯一性约束避免重复。
  - 跨域映射需谨慎评估，建议在同一域内建模后再做跨域集成。
  - 预组合子表适用于头表+明细表的场景，提高数据展示的灵活性。

**更新** 新增的最佳实践包括关系类型的选择指导和筛选条件的性能考虑。