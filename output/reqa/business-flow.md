...
## 流程六：登录与字段权限过滤（REQ-019，auth 落地）

```mermaid
flowchart TB
    subgraph 系统管理员
        A[登录 admin 账号] --> B[系统管理页：新建角色]
        B --> C[按档案域勾选字段可见/可编辑白名单]
        C --> D[新建用户+分配角色]
    end
    subgraph 普通用户
        E[账号密码登录] --> F{权限过滤层}
        F -- 未登录/已禁用 --> G[401 → 重定向登录页]
        F -- 已登录 --> H[档案列表/详情：仅下发可见字段]
        H --> I[编辑：仅可编辑字段可改]
        I --> J[保存：不可编辑字段修改被忽略]
    end
    D -.权限集生效.-> F
...
**系统边界**：过滤在后端序列化层执行（隐藏字段数据不下发）；前端仅按响应呈现，不承担权限职责。
...
| 权限 | 未登录/会话失效/用户被禁用 | 401，前端重定向登录页 |
| 权限 | 伪造请求修改不可编辑字段 | 后端静默忽略该字段，其余字段正常保存 |
| 权限 | 角色在某域零配置（白名单空） | 该域全部字段隐藏，页面呈现空列并提示无权限 |
