...
- **职责**：管理用户身份、角色权限、API开放策略；提供审计日志查询能力；REQ-019 落地：登录会话+角色×档案域字段可见/可编辑控制（白名单制，后端序列化层过滤）
- **边界**：不控制业务逻辑，只做权限校验的中间层；变更日志与对外 API 本期不叠加字段过滤
- **对外接口**：登录/登出、用户 CRUD（新建/禁用/重置密码）、角色 CRUD、角色×域字段权限配置、当前用户会话信息
- **依赖**：modeling（域/字段集作为权限配置对象）、archive（档案接口为过滤执行点）
...
| auth（登录+用户+角色+字段权限过滤，REQ-019） | modeling（域/字段集）+ archive（档案接口过滤点） | P0 | 开发中 |
...
| REQ-019 角色权限与字段可见/可编辑控制 | REQ-019.md | F-201~F-203, F-209~F-210 | 流程六（登录与字段权限） |
...
- **对外接口**：
  - 权限校验接口（供archive调用）
  - API密钥认证（供外部系统鉴权）
  - 登录/登出、用户与角色管理、角色×档案域字段可见/可编辑配置（REQ-019 落地）
- **依赖**：modeling（域/字段集作为权限配置对象）、archive（档案接口为过滤执行点）
...
| 权限模型 | 角色×档案域×字段可见/可编辑（白名单制，REQ-019 替代原角色×档案×操作） | 字段级粒度贴合主数据敏感字段管控；后端序列化层过滤而非前端藏列 |
