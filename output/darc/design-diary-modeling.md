...
## 2026-08-07 v12 — 域内配置表（ConfigTable）+ MAP_VALUE 配置表驱动

### 背景

用户有大量产品编码映射场景（如 D_918_5_884_BM_FH_J_3 按分隔符拆分后映射为中文名称），现有 MAP_VALUE 函数需要在公式里写长串 "D:标准件;B:非标件;..."，维护困难。用户希望改为配置表驱动：提前在管理界面配好映射，公式里只写配置表编码。

### 设计决策

1. **数据模型**：新增 `ConfigTable` 模型（域内轻量级查找表），不参与同步/ER图/字段映射。数据用 JSONField 存储（columns 列定义 + rows 行数据），无需物理建表。
2. **公式引用**：MAP_VALUE 第二参数支持配置表编码（如 `MAP_VALUE(值, "product_type", "默认值")`），自动查域内配置表。第一列=key，第二列=value。向后兼容旧版映射串。
3. **前端入口**：管理表页面（TableList.vue）加「配置表」按钮，跳转到独立配置表管理页（ConfigTables.vue），支持 CRUD + 行数据管理弹窗。
4. **上下文注入**：`_build_context_from_record` 注入 `__domain_id__` 供 MAP_VALUE 查找配置表。

### 验证

迁移 0027 OK✓ 新增 6 测试（41/41 PASS）✓ 全站 114 测试 0 回归✓ vue-tsc 0 errors✓
